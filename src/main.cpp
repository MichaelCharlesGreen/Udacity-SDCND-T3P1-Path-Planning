      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            //find ref_v to use
            // all cars in the sensor fusion data
            for(int i =0; i < sensor_fusion.size(); i++)
            {
              ...
              if(d < (2+4*lane+2) && d > (2+4*lane-2))
              {
                ...
                check_car_s += ((double)prev_size*.02*check_speed); //if using previous points can projet s value out
                ...
                if((check_car_s > car_s) && ((check_car_s-car_s) < 30))
                {
                  ...
                  //ref_vel = 29.5; //mph
                  too_close = true;
                  // If we are in the middle or right lane, slam over to the left lane as fast as we can.
                  // TODO: fix this using finit state machine and frenet
                  if(lane > 0)
                  {
                    lane = 0;
                  }
            ...
            // either we will reference the starting point 03 where the car is at previous paths end point
            double ref_x = car_x;
            double ref_y = car_y;
            ...
            // beginning of edit
            // In frenet add evelnly 30m spaced points ahead of the starting reference
            ...
            for (int i = 1; i <= 50-previous_path_x.size(); i++)
            {
              ...
              double N = (target_dist/(.02*ref_vel/2.2));
